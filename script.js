...
// WebRTC helpers (existing system may use these; left intact)
const rtcSessions = {};
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if(logoutBtn) logoutBtn.addEventListener('click', async () => {
    try { await auth.signOut(); } catch(err){ console.error('signOut error', err); alert('ログアウトに失敗しました: ' + (err && err.message ? err.message : err)); }
  });
...
// Calls minimal (keeps existing behavior from earlier versions)
...
function sendCallRequestFromPopup(){ /* existing call send implementation omitted for brevity; keep as before in your full script */ }
function listenIncomingCalls(myUid){ /* existing incoming call implementation omitted for brevity; keep as before in your full script */ }
function respondToIncomingCall(result){ /* as before */ }
...
// Helper: determine host (simple: page owner must set window._streamHostUid externally if needed)
function isHost(){
  return !!auth.currentUser && auth.currentUser.uid === window._streamHostUid;
}

// Start game (host)
async function startGameByHost(){
  if(!auth.currentUser) return alert('ゲーム開始はログインした主催者のみ可能です');
  if(!isHost()) return alert('ゲームを開始する権限がありません（主催者のみ）');
...
// Open game UI locally and attach listener
...
function renderGameHeader(game){
  ...
  if(auth.currentUser){
    if(game.status === 'lobby'){
      const joinBtn = document.createElement('button'); joinBtn.textContent = '参加希望';
      joinBtn.addEventListener('click', ()=> requestJoinGame(game.id));
      controls.appendChild(joinBtn);
      if(auth.currentUser.uid === game.hostUid){
        const pickBtn = document.createElement('button'); pickBtn.textContent = '参加者から選出して開始';
        pickBtn.addEventListener('click', ()=> pickAndStartGame(game.id));
        controls.appendChild(pickBtn);
      }
    } else if(game.status === 'running'){
      if(auth.currentUser.uid === game.hostUid){
        const endBtn = document.createElement('button'); endBtn.textContent = '強制終了';
        endBtn.addEventListener('click', ()=> endGame(game.id, null));
...
async function pickAndStartGame(gid){
  ...
  await gamesRef.child(gid).child('shogi').set({
    board: initialShogiBoard(),
...
// Render Shogi board and controls
...
// End game, award points to winner
...
function initGameAutoSubscribe(){
  ...
  gamesRef.on('child_changed', snap=>{
    ...
    if(g.status === 'running') openGameUI(g.id, g);
  });
}
