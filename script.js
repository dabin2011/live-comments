...
// 注意: 必ず firebaseConfig と GAS_URL を実環境の値に置き換えてください。
...
// WebRTC helpers
const rtcSessions = {}; // callId -> { pc, localStream, remoteAudioEl, iceListenerRef, timerInterval, callStartTs }
...
async function updateProfile() {
  ...
  } catch (err) { console.error('updateProfile error', err); alert('ユーザー名の更新に失敗しました: ' + (err && err.message ? err.message : err)); }
}
...
function ensurePollListener() {
  pollsRef.child('active').on('value', snap => {
    ...
    localActivePoll = data; renderPollState(data);
  ...
  if (pollTimer) {
    ...
    if (poll.state === 'voting') {
      const updateFn = () => {
        ...
        if (remainingMs <= 0) { if (pollTimer) pollTimer.textContent = '集計中...'; finalizePoll().catch(err => console.error('finalizePoll error', err)); if (_pollTimers.has('active')) { clearInterval(_pollTimers.get('active')); _pollTimers.delete('active'); } return; }
...
// Open game UI and attach listener
function openGameUI(gid, initialObj) {
  ...
  gamesRef.child(gid).on('value', snap => {
    const g = snap.val(); if (!g) { closeGameUI(); return; }
    gameLocalState = g; renderGameState(g);
  });
}
