// script.js — 完全版（ログイン/新規登録/コメント/アンケート/ゲーム/プロフィール画像/モーダル/到着バナー）
// 重要: index.html の <head> で Firebase compat SDK (app/auth/database/storage) を先に読み込んでください。
// また、下の firebaseConfig と GAS_URL を実運用値に置き換えてください。
...
if (typeof firebase === 'undefined') {
  ...
  try { if (!firebase.apps.length) firebase.initializeApp(firebaseConfig); } catch (e) { console.error('firebase initialize error', e); }
}
...
async function updateProfile(){
  try {
    ...
    if (usersRef) usersRef.child(user.uid).child('profile').update({ displayName:newName }).catch(()=>{});
...
async function uploadProfileImage(){
  try {
    ...
    if (!storage) { alert('Storage 未初期化: Firebase Storage を読み込んでください'); return; }
...
function addPollOption(){
  const wrap = el('pollOptionsWrapper');
  if(!wrap) return;
  const input = document.createElement('input');
  input.type='text'; input.className='pollOptionInput'; input.placeholder='選択肢';
...
function createPollFromModal(){
  try {
    ...
    if (pollsRef) {
      ...
      setTimeout(()=>{ finalizePollLocal(poll); }, POLL_DURATION_MS);
    }
  } catch(e){ console.error('createPollFromModal error', e); alert('アンケート作成失敗'); }
}
function finalizePollLocal(poll){
  try { poll.state = 'finished'; renderPollState(poll); setTimeout(()=>{ hidePollUI(); }, POLL_AFTER_FINISH_DISPLAY_MS); } catch(e){ console.error(e); }
}
...
async function startGameByHost(){
  try {
    ...
    if (gamesRef) { await gamesRef.child(gid).set(gameObj); openGameUI(gid, gameObj); } else openGameUI(gid, gameObj);
...
function renderGameHeader(game){
  ...
  const assetsInfo = document.createElement('div'); assetsInfo.style.display='inline-flex'; assetsInfo.style.flexDirection='column'; assetsInfo.style.marginLeft='12px'; assetsInfo.innerHTML = `<div style="font-size:12px;color:#666">駒/盤: デフォルト</div>`; controls.appendChild(assetsInfo);
...
async function requestJoinGame(gid){
  ...
  if (gamesRef) await gamesRef.child(gid).child('players').child(u.uid).set(u);
...
async function pickAndStartGame(gid){
  try {
    ...
    const players = [];
    snap.forEach(ch=>{ const v = ch.val(); if (v && v.uid) players.push(v); });
...
async function renderShogiBoard(gid, shogiState){
  ...
  container.innerHTML = '';
  const boardWrap = document.createElement('div'); boardWrap.className = 'shogiBoard'; boardWrap.style.position='relative'; boardWrap.style.padding='8px'; boardWrap.style.boxSizing='border-box';
  const size = 9;
