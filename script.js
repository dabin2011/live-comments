...
function setupListeners(){
  arrivalsRef.on("child_added", snap=>{ const v=snap.val(); if(v && v.type==="arrival") showArrivalBanner(v.name); snap.ref.remove().catch(()=>{}); });
  ensurePollListener();
  initComments();
}
...
function ensurePollListener(){
  ...
  pollsRef.child("active").on("value", snap=>{
    ...
    if(localActivePoll.state === "voting" && Date.now() > localActivePoll.endsAt) finalizePollIfNeeded();
  });
...
function renderPollState(poll){
  ...
  (poll.options||[]).forEach(o=>{
    ...
    if(poll.state === "voting"){
      ...
      if(uid){ pollsRef.child("active").child("votes").child(uid).once("value").then(snap=>{ const v=snap.val(); if(v && v.opt) markSelectedOption(v.opt); }).catch(()=>{}); }
    } else {
  ...
  if(poll.state === "counting"){
    ...
    if(ex){ ex.classList.add("pulse"); ex.style.opacity = "1"; }
...
function voteOption(optId){
  ...
  voteRef.set({ opt:optId, at:Date.now(), name:user.displayName||user.email }).then(()=>{ myVoteOpt = optId; markSelectedOption(optId); }).catch(err=>console.warn("vote failed", err));
}
  ...
  setTimeout(()=>{
    pollsRef.child("active").child("votes").once("value").then(snap=>{
      ...
      pollsRef.child("active").update({ options: newOptions, state: "showResults", showResultsAt: Date.now() })
        .then(()=>{ setTimeout(()=>{ const pollArea=document.getElementById("pollArea"); if(pollArea) pollArea.classList.add("poll-fadeout"); setTimeout(()=>{ pollsRef.child("active").remove().catch(()=>{}); finalized=false; },600); }, 30000); })
...
function hidePollUI(){ const pollArea=document.getElementById("pollArea"); if(!pollArea) return; pollArea.style.display="none"; pollArea.classList.remove("poll-fadeout"); document.getElementById("pollContent").innerHTML=""; document.getElementById("pollTimer") && (document.getElementById("pollTimer").textContent=""); myVoteOpt=null; }
...
document.getElementById("uploadForm")?.addEventListener("submit", function(e){
  ...
  fetch(GAS_URL, { method:"POST", body:fd }).then(r=>r.text()).then(url=>{ const user=auth.currentUser; if(!user) return alert("ログインしてください"); user.updateProfile({ photoURL:url }).then(()=>{ document.getElementById("avatar").src=url; closeModal('mypageModal'); }).catch(err=>alert("更新失敗："+err.message)); }).catch(err=>alert("アップロード失敗："+err.message));
});
